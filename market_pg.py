...
class PolicyGradient:
	...
	def train(self, max_episode = 1000, max_path_length = 200, verbose = 1):
		env = self.env
		model = self.model
		avg_reward_sum = 0.
...
if __name__ == "__main__":
	...
	envListFilename = sys.argv[2]
	modelFilename = sys.argv[3] if len(sys.argv) > 3 else None
	historyFilename = sys.argv[4] if len(sys.argv) > 4 else None

	codeMap = {}
	envMap = {}
	f = codecs.open(codeListFilename, "r", encoding = "euc-kr", errors='ignore')
	...
	ff = codecs.open(envListFilename, "r", encoding="euc-kr", errors='ignore')
	...
	env = MarketEnv(dir_path = "./data/", target_codes = list(codeMap.keys()), input_codes = list(envMap.keys()), start_date = "2015-08-01", end_date = "2017-07-30", sudden_death = -1.0)
